      ...
      <div className="mt-12">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setLangFilter('cpp')} className={`rounded-md px-3 py-1.5 text-sm border ${langFilter==='cpp' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>C++</button>
            <button onClick={() => setLangFilter('java')} className={`rounded-md px-3 py-1.5 text-sm border ${langFilter==='java' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted'}`}>Java</button>
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {visible.map((q) => (
            <details key={q.id} className="rounded-lg border bg-card open:shadow">
              <summary className="cursor-pointer select-none px-4 py-3 font-medium flex items-center justify-between">
                <span>{q.title} <span className="ml-2 text-xs rounded bg-muted px-2 py-0.5 align-middle">{q.difficulty}</span></span>
                <span className="text-xs text-foreground/60">{q.topics.join(', ')}</span>
              </summary>
              <div className="px-4 pb-4 text-sm space-y-2">
                <p>{q.statement}</p>
                {q.sampleInput && (
                  <div>
                    <div className="text-xs text-foreground/60">Sample Input</div>
                    <pre className="rounded-md bg-muted p-3 overflow-x-auto"><code>{q.sampleInput}</code></pre>
                  </div>
                )}
                {q.sampleOutput && (
                  <div>
                    <div className="text-xs text-foreground/60">Sample Output</div>
                    <pre className="rounded-md bg-muted p-3 overflow-x-auto"><code>{q.sampleOutput}</code></pre>
                  </div>
                )}
              </div>
            </details>
